...
Gui, Show, x557 y353 h256 w418 , Condor2LK_v1.4 ; New GUI Window
Return
...
IniRead, FixedMass, %TaskDir%, Plane, FixedMass
	if (FixedMass = 0)
		msgBox Warning!!! No fixed balast set
...
a := 3 ; temp variable for the loop
s := 3 ; temp variable for the loop 
;***********Loop to determine r************************
...
While a < count
		...
		If Tp1Radius <> %TpRadius% 
			...
			while s < count
				{
				IniRead, TpRadius, %TaskDir%, Task, TpRadius%s%
				s := s + 1
				If Tp1Radius > %TpRadius%
					{
					Tp1Radius := TpRadius
...
IniRead, PolarString, C2LKPolars.txt, %PlaneType%, flaps
FileAppend, %Polarstring% `n, %LKPolarDir%
